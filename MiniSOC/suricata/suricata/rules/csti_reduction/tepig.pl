...
my $url = 'https://www.google.com.tw';
...
foreach $line (@lines) {
  ...
  if ($line =~ m/($ip_regex)/) {
    my $ip = $1;
    if(lc($ids) ne "cisco") { 
            print "alert ip \$HOME_NET any <> $ip any (msg:\"Suspicious ip $ip has been detected!\"; classtype:suspicious-ip; sid:$sid; rev:1;)\n";
    } else {
    ...
    if(lc($ids) ne "cisco") { 
      print "alert udp \$HOME_NET any -> any 53 (msg:\"Suspicious host name $domainname has been detected!\"; content:\"";
      print "$dnslookup";
      print "\"; nocase; classtype:suspicious-host-name; sid:$sid; rev:1;)\n";
    } else {
      print "signatures $sid 0\n";
      print "sig-description\n";
